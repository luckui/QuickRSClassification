...
output_model = "temp/rf_model.pkl"  # 输出模型文件
output_report = "temp/accuracy_report.txt"  # 输出精度报告
...
def load_sample_data(sample_file):
    ...
    # 过滤掉未标注的样本（ClassName不为空，或者根据实际情况调整）
    # 注意：0也是有效的类别标签！
...
def generate_report(results, output_path):
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        f.write("模型参数\n")
        f.write("-"*70 + "\n")
        f.write(f"算法: 随机森林 (Random Forest)\n")
...
def main():
    ...
    try:
        ...
        # 2. 训练模型并评估
        model, results = train_random_forest(X, y, feature_names)
