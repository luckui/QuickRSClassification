...
output_model = "temp/lgbm_gridsearch_model.pkl"  # 输出最优模型文件
output_report = "temp/lgbm_gridsearch_report.txt"  # 输出网格搜索报告
output_best_params = "temp/lgbm_best_params.json"  # 输出最优参数JSON文件

# 网格搜索参数范围
param_grid = {
    'n_estimators': [50, 100, 200],  # 树的数量
    'max_depth': [10, 20, -1],  # 最大深度（-1表示无限制）
    'learning_rate': [0.01, 0.05, 0.1],  # 学习率
    'num_leaves': [15, 31, 63],  # 叶子节点数
    'min_child_samples': [10, 20, 30],  # 叶节点最小样本数
...
def grid_search_lgbm(X, y, feature_names, param_grid, cv_folds, scoring):
    """
    使用网格搜索寻找LightGBM最优超参数
...
def main():
    ...
    try:
        ...
        # 2. 网格搜索寻找最优超参数
        best_model, grid_search, results = grid_search_lgbm(
